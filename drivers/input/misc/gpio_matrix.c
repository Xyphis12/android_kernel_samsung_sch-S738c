...
static void report_key(struct gpio_kp *kp, int key_index, int out, int in)
{
	...
	printk(KERN_INFO "Inside report_key\n");
	...
	(defined(CONFIG_SEC_DUMP) || defined(CONFIG_DPRAM))
		if (dump_enable_flag != 0)
			printk(KERN_INFO "[KEY] : %s, %s\n",
				keycode == KEY_VOLUMEDOWN ? "Vol_Dn" : "Vol_Up",
					pressed ? "Pre_d" : "Rel_d");
...
#endif
#endif /*endif for CONFIG_MACH_AMAZING_CDMA)*/
...
static irqreturn_t gpio_keypad_irq_handler(int irq_in, void *dev_id)
{
	...
	struct gpio_event_matrix_info *mi = kp->keypad_info;
	unsigned gpio_keypad_flags = mi->flags;
	printk(KERN_INFO"Inside gpio_keypad_irq_handler\n");
	if (!kp->use_irq) {
...
#if !defined(CONFIG_MACH_KYLE) && !defined(CONFIG_MACH_AMAZING)\
	&& !defined(CONFIG_MACH_AMAZING_CDMA) && !defined(CONFIG_MACH_TREBON)
