...
#ifdef CONFIG_MACH_AMAZING_CDMA
#define LOCKUP_FOURTH_KEY KEY_POWER
#endif
...
void sec_check_crash_key(int keycode, u8 keypress)
{
	...
	static bool vol_up, home, set_timer;
	...
	if ((keycode == LOCKUP_FIRST_KEY) || (keycode == LOCKUP_FOURTH_KEY)) {
		if (keypress) {
			if ((keycode == LOCKUP_FIRST_KEY))
				vol_up = true;
			if ((keycode == LOCKUP_FOURTH_KEY))
				home = true;
			if (vol_up == true && home == true) {
