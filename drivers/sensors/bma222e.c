...
#endif
#if defined(MACH_AMAZING3G) || defined(CONFIG_MACH_AMAZING_CDMA)
...
#endif
/*
Some times Probe fails in SMD test, as we are getting the Chip Id incorrect,
This happens so, because of the timing Issue of calling yas_bma_init_library
from ioctl,resume API's ,in this particular cass the chip may go to Extended
addressing mode. So, the Chip ID read is from Extended mode.So the below code
is for comming out of extended mode and then read the proper chip ID
*/
