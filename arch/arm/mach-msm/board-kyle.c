...
{
	/* Active Low */
	return (gpio_get_value(GPIO_EAR_GND)) ^ 1;
...
#else
	return ((gpio_get_value(GPIO_JACK_S_35))^1) && (get_sec_gnd_jack_state());
#endif
}
...
#if defined(CONFIG_MACH_KYLE_I)
static unsigned ear_gnd_gpio[] = {
	GPIO_CFG(GPIO_EAR_GND, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
		GPIO_CFG_2MA),
};
#endif
...
#if defined(CONFIG_MACH_KYLE_I)
	gpio_tlmm_config(ear_gnd_gpio[0], GPIO_CFG_ENABLE);
#endif
